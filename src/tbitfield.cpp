...
{/ присваивание //возвр объект типа класс //
	//объект по ссылке поменять нельзя
	//меняем текущее состояние,bf не меняется
{BitLen = bf.BitLen; 
MemLen = bf.MemLen; 
delete []pMem; 
pMem = new TELEM [MemLen]; 
for (int i=0;i<MemLen;i++) 
pMem[i] = bf.pMem[i]; return *this; 
}
...
{//формат послед из 0 и1
	//нач пробелы игнорируются
//при получ не 0 или 1 -завершение ввода
{int i=0; char ch; 
do { istr >> ch; } while (ch !=' '); 
while (1) 
{ 
